-- =====================================================
-- odd.v1_fetch_user_liked_posts_feed: 사용자가 좋아요한 포스트 피드 조회 함수
-- =====================================================
-- 
...
-- 
-- 사용 위치:
--   - ProfilePage의 좋아요 탭에서 사용자가 좋아요한 포스트 피드 조회 시 호출
--   - 프론트엔드: supabase.schema('odd').rpc('v1_fetch_user_liked_posts_feed', {...})
-- 
-- 실행 방법:
--   psql "postgresql://postgres.xyqpggpilgcdsawuvpzn:ZNDqDunnaydr0aFQ@aws-0-ap-northeast-2.pooler.supabase.com:5432/postgres" -f docs/sql/047_v1_fetch_user_liked_posts_feed.sql
-- 
-- =====================================================
-- 1. 사용자가 좋아요한 포스트 피드 조회 함수
-- =====================================================

DROP FUNCTION IF EXISTS odd.v1_fetch_user_liked_posts_feed(text, integer, integer);

CREATE OR REPLACE FUNCTION odd.v1_fetch_user_liked_posts_feed(
    p_username text,
...
AS $$
/*
 * 함수 설명: 특정 사용자가 좋아요한 포스트의 피드를 조회합니다.
 *           일반 포스트, 커뮤니티 공지, 피드백, 프로젝트 생성 정보를 포함하며
 *           모든 피드를 시간순으로 정렬하여 반환합니다.
 * 
 * 매개변수:
 ...
 * 
 * 반환값:
 *   - 사용자가 좋아요한 포스트의 피드 목록 (작성자 정보, 인터랙션 상태, 피드 타입별 추가 정보 포함)
 * 
 * 보안:
...
BEGIN
    ...
    -- 사용자가 좋아요한 포스트 피드 조회 쿼리 (UNION ALL 사용)
    RETURN QUERY
    WITH liked_posts AS (
        -- 좋아요한 포스트 ID 목록
        SELECT pl.post_id
        FROM odd.tbl_post_likes pl
        WHERE pl.user_id = v_target_user_id
    ),
    unified_posts AS (
        ...
        SELECT 
            ...
            true AS is_liked,  -- 좋아요한 포스트이므로 항상 true
        ...
        INNER JOIN liked_posts lp ON p.id = lp.post_id
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Error in v1_fetch_user_liked_posts_feed: %', SQLERRM;
END;
$$;
...
GRANT EXECUTE ON FUNCTION odd.v1_fetch_user_liked_posts_feed TO authenticated;
GRANT EXECUTE ON FUNCTION odd.v1_fetch_user_liked_posts_feed TO anon;

-- =====================================================
-- 3. 코멘트 추가
-- =====================================================

COMMENT ON FUNCTION odd.v1_fetch_user_liked_posts_feed IS '특정 사용자가 좋아요한 포스트의 피드를 조회하는 함수. 일반 포스트, 커뮤니티 공지, 피드백, 프로젝트 생성 정보를 시간순으로 정렬하여 반환합니다.';
