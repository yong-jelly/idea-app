-- =====================================================
-- odd.v1_fetch_post_detail: 포스트 상세 조회 함수
-- =====================================================
-- 
-- 포스트 ID로 포스트 상세 정보를 조회합니다.
-- 통합 피드와 동일한 구조로 반환하여 일관성을 유지합니다.
-- 커뮤니티 공지, 피드백, 프로젝트 생성 정보, 일반 포스트를 모두 지원합니다.
-- 
-- 사용 위치:
--   - PostDetailPage에서 포스트 상세 조회 시 호출
--   - 프론트엔드: supabase.schema('odd').rpc('v1_fetch_post_detail', {...})
-- 
-- 실행 방법:
--   psql "postgresql://postgres.xyqpggpilgcdsawuvpzn:ZNDqDunnaydr0aFQ@aws-0-ap-northeast-2.pooler.supabase.com:5432/postgres" -f docs/sql/037_v1_fetch_post_detail.sql
-- 
-- =====================================================
-- 1. 포스트 상세 조회 함수
-- =====================================================
...
-- 기존 함수 삭제 (필드 변경 시 필요)
DROP FUNCTION IF EXISTS odd.v1_fetch_post_detail(uuid);

CREATE OR REPLACE FUNCTION odd.v1_fetch_post_detail(
    p_post_id uuid
)
RETURNS TABLE (
    -- 포스트 기본 정보
...
AS $$
/*
 * 함수 설명: 포스트 ID로 포스트 상세 정보를 조회합니다.
 *           통합 피드와 동일한 구조로 반환하여 일관성을 유지합니다.
 *           일반 포스트, 커뮤니티 공지, 피드백, 프로젝트 생성 정보를 모두 지원합니다.
 * 
 * 매개변수:
 *   - p_post_id: 포스트 ID (필수)
 * 
 * 반환값:
 *   - 포스트 상세 정보 (작성자 정보, 인터랙션 상태, 피드 타입별 추가 정보 포함)
 * 
 * 보안:
...
BEGIN
    ...
    IF p_post_id IS NULL THEN
        RAISE EXCEPTION '포스트 ID는 필수입니다';
    END IF;
    
    -- 포스트 상세 조회 쿼리 (UNION ALL 사용)
    RETURN QUERY
    WITH unified_posts AS (
        -- 1. 일반 포스트 (text, project_update, milestone, feature_accepted, project_created)
        SELECT 
            p.id,
            p.author_id,
            p.type,
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Error in v1_fetch_post_detail: %', SQLERRM;
END;
$$;
...
GRANT EXECUTE ON FUNCTION odd.v1_fetch_post_detail TO authenticated;
GRANT EXECUTE ON FUNCTION odd.v1_fetch_post_detail TO anon;

-- =====================================================
-- 3. 코멘트 추가
-- =====================================================

COMMENT ON FUNCTION odd.v1_fetch_post_detail IS '포스트 ID로 포스트 상세 정보를 조회하는 함수. 통합 피드와 동일한 구조로 반환하여 일관성을 유지합니다. 일반 포스트, 커뮤니티 공지, 피드백, 프로젝트 생성 정보를 모두 지원합니다.';
