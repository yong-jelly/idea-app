...
export function Header() {
  ...
  // 특정 페이지일 때 모바일에서 헤더 숨김
  // - 프로젝트 관련 페이지: /project/:id, /project/:id/community 등
  // - 피드 상세 페이지: /:username/status/:postId
  const isProjectPage = location.pathname.match(/^\/project\/[^/]+/);
  const isPostDetailPage = location.pathname.includes("/status/");
  const shouldHideOnMobile = !!isProjectPage || !!isPostDetailPage;
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6">
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Mobile Menu Button - 숨김 (모바일에서는 다크 모드만 표시) */}
          </div>
        </div>
